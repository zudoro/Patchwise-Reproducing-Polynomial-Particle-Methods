...
SUBROUTINE GEOSTIF(KG)
	REAL*8, INTENT(OUT) :: KG(INT(0.5*DOF*(DOF+1)))
	TYPE(INTPATCH) :: INTPATCHGROUP(NUMPATCHES,NUMPATCHES,NUMINTSUBPATCH)
	TYPE(INT2D) :: SF_INDX_I, SF_INDX_J
	INTEGER :: I, J, KK, NINTPATCHGROUP(NUMPATCHES,NUMPATCHES), NDX(3,DOF)
  REAL*8 :: X(NGAUSSPT), W(NGAUSSPT)
  
  PRINT*, ''
  PRINT*, 'ASSEMBLE THE GEOMETRICAL STIFFNESS MATRIX'
  ...
  DO I=1, DOF
  	SF_INDX_I = INT2D(NDX(2,I), NDX(3,I))
		DO J = I, DOF
			SF_INDX_J = INT2D(NDX(2,J), NDX(3,J))
    !--------------- SUB MASS MATRIX -------------------
      KG(KK) = INTEGRATION_KG(BGMESH(NDX(1,I)), BGMESH(NDX(1,J)), SF_INDX_I, SF_INDX_J, &
					     INTPATCHGROUP(NDX(1,I),NDX(1,J),:), NINTPATCHGROUP(NDX(1,I),NDX(1,J)), IORDER, X, W, NGAUSSPT)
			KK = KK + 1
			IF (DABS(KG(KK)).LE.EPS) THEN
				KG(KK) = 0.D0
			ENDIF
		ENDDO
  ENDDO
	
END SUBROUTINE GEOSTIF

SUBROUTINE MASS(M)

    REAL*8, INTENT(OUT) :: M(INT(0.5*DOF*(DOF+1)))
    TYPE(INTPATCH) :: INTPATCHGROUP(NUMPATCHES,NUMPATCHES,NUMINTSUBPATCH)
    TYPE(INT2D) :: SF_INDX_I, SF_INDX_J
    INTEGER :: I, J, KK, NINTPATCHGROUP(NUMPATCHES,NUMPATCHES), NDX(3,DOF)
    REAL*8 :: X(NGAUSSPT), W(NGAUSSPT)
     ...
     PRINT *,
     PRINT *, 'ASSEMBLE THE MASS MATRIX'
...
!            AND INTERPOLATION DEGREE FOR EACH STIFFNESS ELEMENTS
    ...
    DO I = 1, DOF
		...
		DO J = I, DOF
			SF_INDX_J = INT2D(NDX(2,J), NDX(3,J))
    !--------------- SUB MASS MATRIX -------------------
        M(KK) = INTEGRATION_M(BGMESH(NDX(1,I)), BGMESH(NDX(1,J)), SF_INDX_I, SF_INDX_J, &
					   INTPATCHGROUP(NDX(1,I),NDX(1,J),:), NINTPATCHGROUP(NDX(1,I),NDX(1,J)), IORDER, X, W, NGAUSSPT)
		KK = KK + 1
		ENDDO
    ENDDO
