...
!	Material Coefficients
	...
	CHARACTER(LEN=4), PARAMETER :: BCTYPE = 'SSSS' ! CLAM - CLAMPED(FIXED), HARD - HARD SIMPLY SUPPORTED
											! SOFT - SOFT SIMPLY SUPPORTED, FREE - JUST FREE
														
	REAL*8, PARAMETER :: L=1.0D0	! LENGTH OF THE PLATE ALONG X-AXIS 	
	REAL*8, PARAMETER :: h=0.010D0*L	! (d) THICKNESS OF THE PLATE
	REAL*8, PARAMETER :: HOOKS_K=0.0D0	! FOUNDATION MODULUS
	REAL*8, PARAMETER :: LOAD_Q=1.0D0	! UNIFORMLY DISTRBUTED LOAD

	! ISOTROPIC : E1=E2=E, V12=V21=V, G12=G13=G23=G = E/2(1+V)
		
	REAL*8, DIMENSION(1:2), PARAMETER :: E=10920.0D0*(/1.0D0, 1.0D0/) 	! YOUNG'S MODULUS {E1, E2}, STEEL
	REAL*8, DIMENSION(1:2), PARAMETER :: V=0.30D0*(/1.0D0, 1.0D0/)		! POISSON RATION {V12, V21}
	...
	REAL*8, PARAMETER :: GAMMA = DSQRT(12.0D0*KS + (h/L)**2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	TYPE(TRIPATCH), PARAMETER :: TRI0 = TRIPATCH(PT0,PT0,PT0)
	TYPE(TRIPATCH), PARAMETER :: MSTTRI = TRIPATCH(POINT2D(-1.D0,-1.D0), POINT2D(1.D0,-1.D0),POINT2D(-1.D0,1.D0))
	TYPE(TRIPATCH), PARAMETER :: UNTTRI = TRIPATCH(POINT2D(0.D0,0.D0), POINT2D(1.D0,0.D0),POINT2D(0.D0,1.D0))
	TYPE(RECPATCH), PARAMETER :: MSTREC = RECPATCH(POINT2D(-1.D0,-1.D0), POINT2D(1.D0,-1.D0), POINT2D(1.D0,1.D0),&
																								 POINT2D(-1.D0,1.D0))
	TYPE(RECPATCH), PARAMETER :: UNTREC = RECPATCH(POINT2D(0.D0,0.D0), POINT2D(1.D0,0.D0), POINT2D(0.D0,1.D0),&
																								 POINT2D(1.D0,1.D0))
