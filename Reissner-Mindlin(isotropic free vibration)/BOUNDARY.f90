...
SUBROUTINE REDUCEMATRIX(THIN_K, THIN_M, K, M, REDUCEDOF, BD_MASK)
	INTEGER, INTENT(IN) :: REDUCEDOF
	LOGICAL, INTENT(IN) :: BD_MASK(3*DOF)
	TYPE(SUBMAT), INTENT(IN) :: K
	REAL*8, INTENT(IN) :: M(INT(0.5*DOF*(DOF+1)))
	REAL*8, INTENT(OUT) :: THIN_K(REDUCEDOF, REDUCEDOF), THIN_M(REDUCEDOF, REDUCEDOF)
	
	REAL*8, ALLOCATABLE :: FAT_K(:,:), FAT_M(:,:)
	INTEGER :: I, J, II, JJ, KK
	
	ALLOCATE(FAT_K(3*DOF,3*DOF), FAT_M(3*DOF, 3*DOF))
	
	!----- CONSTRUCT MATRIX [K] AND [M] WITOUHT ROWS AND COLUMNS CORRESPONDING TO BOUNDARY NODES -----

    PRINT *,
    PRINT *, 'CONSTRUCT REDUCED MATRIX [K] AND [M] ASSOCIATED WITH BOUNDARY CONDITION'
	...
	IF (REDUCEDOF.NE.UBOUND(THIN_K,1) .OR. REDUCEDOF.NE.UBOUND(THIN_K,2)) THEN
