...
!
  ...
  IF ( ALL ( A1(1:N) == AINT ( A1(1:N) ) ) .AND. &
    ...
    END DO
  ELSE IF ( ALL ( ABS ( A1(1:N) ) < 1000000.0D0 ) .AND. &
            ALL ( ABS ( A2(1:N) ) < 1000000.0D0 ) ) THEN
    DO I = 1, N
...
!
    ...
    IF ( DABS(SCALE - 0.0D0).LE.5.0D-15 ) THEN
...
!
  ...
  IF ( P /= 0.0D0 ) THEN
    ...
    DO
      ...
      IF ( DABS(T-4.0D0).LE.5.0D-15 ) THEN
        EXIT
      END IF
...
SUBROUTINE PWMTD(A, M, W, IND)
	REAL*8, INTENT(IN) :: M(:,:)
	REAL*8, INTENT(INOUT) :: A(:,:)
	REAL*8, INTENT(OUT) :: W(:)
	INTEGER, INTENT(OUT) :: IND(:)
	
	REAL*8, ALLOCATABLE :: OLDX(:), NEWX(:),LAMDA(:), ERROR(:)
	REAL*8 :: TOL
	...
	ALLOCATE(OLDX(N), NEWX(N), ERROR(N), LAMDA(N), SORTIND(N))

	OLDX(:) = 1.0D0; W(:)=0.D0; IND(:) = 0
	FORALL (I=1:N) SORTIND(I)=I
	MX = (/1, 0/)
...
!	  PRINT*, 'LAMDA', LAMDA(K)
!	ENDDO
	CALL QUICKSORT(N,W,SORTIND)
	LAMDA(:) = W(:)
	FORALL (K=1:N) W(K) = LAMDA(SORTIND(K))
!	W(:) = LAMDA(:)
	PRINT*, ""
	PRINT*, "FINAL ITERATION NUMBER : ", I
	PRINT*, ""
	DEALLOCATE(OLDX, NEWX, LAMDA, ERROR)
